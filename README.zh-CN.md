...
- **brief 是参谋话术质量的上限**：开会/面试前把背景资料写进去——
  菜单「会议背景（人物画像）」子菜单可编辑当前背景；结构化模板见
  `docs/brief-templates/`（面试：公司与岗位 JD、简历要点、各高频
  问题口径与薪资底线、逆问预备；商务会议：目标底线、进度与数字
  口径、风险、对方情况）。brief 全文走 prompt cache，写细不心疼钱。
- **人物画像库**：`~/AudioGateway/profiles/` 下每人一份画像
  （`王海云.md` 这样按人命名，内容按面试模板填），菜单子菜单里
  一键切换——切换即把 brief.md 指向该画像（符号链接），当前生效
  的画像带勾选标记；会中切换热重载即刻生效。切换前手写的临时
  背景会自动抢救进画像库，绝不丢失。
